...
class AddStudent(Frame):
    ...
    def to_add_student(self):
        # created a list of all entries of user
        ...
        entry_size = self.get_entry_size(student_data)
        self.clear_text()

        # if all entries were filled
        if entry_size == 6:
        ...
        # if not all entries were filled
        else:
            self.response.configure(text="Insufficient data!", fg="#F04C42")
            self.response.place_configure(x=690, y=107)

    # get the number of entries
    def get_entry_size(self, student_data):
        size = 0
        for data in student_data:
            size = (size + 1 if data != "" else size)
        return size
